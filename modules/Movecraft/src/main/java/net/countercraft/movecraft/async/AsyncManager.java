    ...
    private void processCruise() {
        for (Craft craft : CraftManager.getInstance()) {
            ...
            for (Entity entity : craft.getWorld().getNearbyEntities(craft.getHitBox().getMidPoint().toBukkit(craft.getWorld()),
                ...
                if (((TNTPrimed)entity).getFuseTicks() <= 20) { //better safe than sorry
                    skipTick = true;
                    break;
                }
            ...
            if (skipTick) {
                Bukkit.getServer().broadcastMessage("Skipping cruise jump!");
                continue;
            }

            Bukkit.getServer().broadcastMessage("Cruise jump successful!");
